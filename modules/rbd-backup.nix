...
in
{
  ...
  config = lib.mkIf (cfg.enable && settings.hostName == cfg.designatedHost) {
    ...
    systemd.services.rbd-backup = {
      description = "Incremental RBD backup to S3 via rclone (zero local staging)";
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "rbd-backup" ''
          ...
          # Set high single-upload cutoff so rclone streams via single PUT
          # up to the S3 API limit of 5 GB.  Incremental diffs are almost always
          # well under this; if a full export exceeds it, rclone may fall back
          # to multipart with temporary files.
          ...
          for IMG in $IMAGES; do
            ...
            if [ -z "$LAST_SNAP" ]; then
              echo "No previous backup snapshot — performing FULL export stream"
              if [ "$COMPRESS" = "true" ]; then
                ${pkgs.ceph}/bin/rbd export "$POOL/$IMG" - | \
                  ${pkgs.zstd}/bin/zstd -19 -c | \
                  ${pkgs.rclone}/bin/rclone rcat "$UPLOAD_PATH/$IMG-$DATE.zst"
              else
                ${pkgs.ceph}/bin/rbd export "$POOL/$IMG" - | \
                  ${pkgs.rclone}/bin/rclone rcat "$UPLOAD_PATH/$IMG-$DATE"
              fi
            else
              echo "Performing INCREMENTAL export stream from snapshot $LAST_SNAP"
              if [ "$COMPRESS" = "true" ]; then
                ${pkgs.ceph}/bin/rbd export-diff --from-snap "$LAST_SNAP" "$POOL/$IMG" - | \
                  ${pkgs.zstd}/bin/zstd -19 -c | \
                  ${pkgs.rclone}/bin/rclone rcat "$UPLOAD_PATH/$IMG-$DATE.diff.zst"
              else
                ${pkgs.ceph}/bin/rbd export-diff --from-snap "$LAST_SNAP" "$POOL/$IMG" - | \
                  ${pkgs.rclone}/bin/rclone rcat "$UPLOAD_PATH/$IMG-$DATE.diff"
              fi
            fi
